...
export const ProposalPriceBreakdownSheet = ({ proposalId, vendorName, submissionDate, evaluationScore, rank }: ProposalPriceBreakdownSheetProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const toastHandlers = useToastHandler();

  const { data: priceBreakdownData, isLoading, error } = useQuery<ProposalPriceBreakdownResponse>({
  ...
  const totalAmount = priceBreakdownItems.reduce((sum, item) => sum + (item.subtotal || 0), 0);

  // Flatten price breakdown items for table display
  const flattenPriceItems = (items: ProposalPriceAction[], level = 0): (ProposalPriceAction & { level: number; itemNumber: string })[] => {
  ...
  const columns: ColumnDef<ProposalPriceAction & { level: number; itemNumber: string }>[] = [
    {
      ...
      <SheetContent className="w-full sm:max-w-4xl p-0 overflow-y-auto ">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-8 mb-6">
            <div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-1">Submitted by</p>
              <p className="text-base font-medium text-gray-900 dark:text-slate-200">
                {vendorName || "Zenith Solutions"}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-1">Submission Date</p>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            <div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-1">Evaluation Score</p>
              <p className="text-base text-gray-900 dark:text-slate-200">
                {evaluationScore || "87%"}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-1">Rank</p>
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-gray-500">Loading price breakdown data...</div>
            </div>
          ) : error ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-red-500">Failed to load price breakdown data</div>
            </div>
          ) : (
            <>
              <DataTable
                ...
                classNames={{
                  ...
                  tHead: "text-sm font-medium text-gray-700 dark:text-gray-300 px-4 py-3 text-left border-b",
                ...
                emptyPlaceholder={
                  <div className="flex items-center justify-center py-12">
                    <div className="text-gray-500 dark:text-slate-200">No price breakdown data available</div>
                  </div>
                }
              />
              ...
              <div className="border-t-2 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
                <div className="flex justify-between items-center px-4 py-4">
                  <span className="text-lg font-semibold text-gray-900 dark:text-slate-200">Total</span>
          ...
          <div className="flex justify-end gap-3 mt-8">
            ...
            >
              <Download className="h-4 w-4" />
              {exportPriceBreakdownMutation.isPending ? "Downloading..." : "Download"}
            </Button>
            <Button 
